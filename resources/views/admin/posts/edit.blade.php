...
<body class="flex min-h-screen">
    ...
    <div class="flex-1 p-8">
        <h1 class="text-2xl font-bold text-gray-800 mb-8">Modifier l'article</h1>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form method="POST" action="{{ route('admin.posts.update', $post->id) }}" enctype="multipart/form-data" id="editForm">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="titre" value="{{ $post->titre }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">
                </div>

                <div class="mb-6">
                    <label class="block text-gray-700 text-sm font-semibold mb-2">Catégorie</label>
                    <input type="text" name="categorie" value="{{ $post->categorie }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="Ex: React, CSS, TypeScript...">
                </div>
                ...
                <div class="mb-6">
                    ...
                    @if($post->image)
                        <div class="mb-3">
                            ...
                            <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->titre }}" class="max-h-48 rounded-lg border border-gray-200 object-cover">
                        </div>
                    @endif
                    ...
                    <p class="text-sm text-gray-500 mt-2">JPEG, PNG, GIF ou WebP — maximum 10 Mo. Laissez vide pour conserver l'image actuelle.</p>
                ...
                <div class="flex justify-end">
                    ...
                    <button type="submit" class="bg-purple-500 hover:bg-purple-600 text-white font-semibold py-3 px-6 rounded-lg transition">Mettre à jour</button>
                </div>
            </form>
        </div>
    </div>

    @include('admin.posts.partials.quill', ['formId' => 'editForm', 'initialContent' => old('contenu', $post->contenu)])
